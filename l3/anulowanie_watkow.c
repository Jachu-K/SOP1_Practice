...
void kick_student(studentsList_t *studentsList,int idx);
...
int main(int argc, char **argv)
{
    ...
    // inicjalizacja permutacji do wyrzucania studentow
    int tab[studentsCount];
    for (int i=0;i<studentsCount;i++) {
        tab[i]=i;
    }
    for (int i=0;i<studentsCount;i++) {
    ...
    do
    {
        ...
        if (clock_gettime(CLOCK_REALTIME, &current))
            ERR("Failed to retrieve time!");
        kick_student(&studentsList,tab[ile_wyrzuconych]);
        ile_wyrzuconych++;
